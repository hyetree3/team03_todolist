"""Discord OAuth2 플로우 — 회원가입/설정 화면의 "Discord 연동" 버튼용.

google_calendar.py의 OAuth 플로우와 같은 목적, 같은 구조다: 사용자가 숫자 ID를 직접
몰라도 되도록, 인증 한 번으로 (1) 진짜 Discord 숫자 ID를 자동으로 알아내고
(2) applications.commands 스코프를 함께 요청해서 User-Install 앱 설치까지 같이
이뤄지게 한다 (이후 예약 DM이 막히지 않으려면 이 설치가 필요함, README 참고).
"""
import os
