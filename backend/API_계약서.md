...
```
- `completed_count`: 그 기간에 완료된 할일 개수. **할일별 최종 상태 기준**이라, 완료→취소→재완료를
  몇 번 반복했든 최종적으로 완료 상태면 그 할일이 마지막으로 완료 처리된 날짜로 1건만 잡힌다
  (도중에 취소한 이력은 안 셈, 최종적으로 취소 상태로 끝났으면 아예 집계 안 됨)
