"""알림 문구 생성 (kakao 파트가 만든 문구를 backend로 이식함).

원본 스펙: "users 테이블 alarm_style 컬럼 ... love / normal / nagging ... 각각
사랑스럽게/평범하게/잔소리 스타일에 맞춰서 문구를 랜덤으로 생성 알람 전송".

todos.category(중요/업무/개인/기타, 한국어 문자열 그대로 들어옴)에 따라 같은 alarm_style
안에서도 문구를 다르게 준다. category가 없거나(None) 저 네 가지 중 하나가 아니면
"기타" 취급한다.
"""
import random
